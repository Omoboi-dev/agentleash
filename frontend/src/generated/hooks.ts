...
export function useLeash_CreatePolicy() {
  ...
  const call = useCallback(async (functionArgs: ClarityValue[] = []) => {
    ...
    try {
      ...
      const fn = (contracts as any).leash_createPolicy;
...
export function useLeash_Deposit() {
  const [data, setData] = useState<unknown>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<Error | null>(null);
  const [txid, setTxid] = useState<string | null>(null);
  ...
  const call = useCallback(async (functionArgs: ClarityValue[] = []) => {
    ...
    try {
      // Use the generated contract function from the index
      const fn = (contracts as any).leash_deposit;
...
export function useLeash_Pay() {
  ...
  const call = useCallback(async (functionArgs: ClarityValue[] = []) => {
    ...
    try {
      // Use the generated contract function from the index
      const fn = (contracts as any).leash_pay;
...
export function useLeash_Revoke() {
  const [data, setData] = useState<unknown>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<Error | null>(null);
  const [txid, setTxid] = useState<string | null>(null);
  ...
  const call = useCallback(async (functionArgs: ClarityValue[] = []) => {
    ...
    try {
      // Use the generated contract function from the index
      const fn = (contracts as any).leash_revoke;
...
export function useLeash_SetActive() {
  const [data, setData] = useState<unknown>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<Error | null>(null);
  const [txid, setTxid] = useState<string | null>(null);
  ...
  const call = useCallback(async (functionArgs: ClarityValue[] = []) => {
    ...
    try {
      // Use the generated contract function from the index
      const fn = (contracts as any).leash_setActive;
...
export function useLeash_SetLimits() {
  const [data, setData] = useState<unknown>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<Error | null>(null);
  const [txid, setTxid] = useState<string | null>(null);
  ...
  const call = useCallback(async (functionArgs: ClarityValue[] = []) => {
    ...
    try {
      // Use the generated contract function from the index
      const fn = (contracts as any).leash_setLimits;
...
export function useLeash_SetRecipient() {
  const [data, setData] = useState<unknown>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<Error | null>(null);
  const [txid, setTxid] = useState<string | null>(null);
  ...
  const call = useCallback(async (functionArgs: ClarityValue[] = []) => {
    ...
    try {
      // Use the generated contract function from the index
      const fn = (contracts as any).leash_setRecipient;
...
export function useLeash_Withdraw() {
  const [data, setData] = useState<unknown>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<Error | null>(null);
  const [txid, setTxid] = useState<string | null>(null);
  ...
  const call = useCallback(async (functionArgs: ClarityValue[] = []) => {
    ...
    try {
      // Use the generated contract function from the index
      const fn = (contracts as any).leash_withdraw;
...
export function useLeash_GetAgents() {
  const [data, setData] = useState<unknown>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<Error | null>(null);
  const [txid, setTxid] = useState<string | null>(null);
  ...
  const call = useCallback(async (functionArgs: ClarityValue[] = []) => {
    ...
    try {
      // Use the generated contract function from the index
      const fn = (contracts as any).leash_getAgents;
...
export function useLeash_GetAvailable() {
  const [data, setData] = useState<unknown>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<Error | null>(null);
  const [txid, setTxid] = useState<string | null>(null);
  ...
  const call = useCallback(async (functionArgs: ClarityValue[] = []) => {
    ...
    try {
      // Use the generated contract function from the index
      const fn = (contracts as any).leash_getAvailable;
...
export function useLeash_GetPolicy() {
  const [data, setData] = useState<unknown>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<Error | null>(null);
  const [txid, setTxid] = useState<string | null>(null);
  ...
  const call = useCallback(async (functionArgs: ClarityValue[] = []) => {
    ...
    try {
      // Use the generated contract function from the index
      const fn = (contracts as any).leash_getPolicy;
...
export function useLeash_IsRecipientAllowed() {
  const [data, setData] = useState<unknown>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<Error | null>(null);
  const [txid, setTxid] = useState<string | null>(null);
  ...
  const call = useCallback(async (functionArgs: ClarityValue[] = []) => {
    ...
    try {
      // Use the generated contract function from the index
      const fn = (contracts as any).leash_isRecipientAllowed;
